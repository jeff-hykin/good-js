...
const testsFolder = `${FileSystem.thisFolder}/../source/test`
const logsFolder = `${FileSystem.thisFolder}/../logs/flat_tests`
...
for (const each of await FileSystem.recursivelyListItemsIn(testsFolder)) {
    if (each.isFile) {
        ...
        try {
            var { success } = await run("deno", "run", "--allow-all", "--no-lock", each.path, Out(Overwrite(logPath)), Env({NO_COLOR:"true"}))
        } catch (error) {
